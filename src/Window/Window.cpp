...
void Patata::Engine::EngineImpl::CreateGameWindow(const std::string &Title,
  ...
  GameWindow = SDL_CreateWindow(PatataWindowTitle.c_str(),
                                SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED,
                                WindowWidth, WindowHeight, flags);

  if (!GameWindow) {
    Patata::Log::FatalErrorMessage("Window cannot be created", SDL_GetError(),
                                   Config);
    exit(1);
  }
