...
void
Patata::Log::OpenGLInfo (const YAML::Node & CONFIG)
{
  ...
  fast_io::io::println (PATATA_TERM_BOLD,
                        "  Graphics Backend : ", PATATA_TERM_RESET,
                        PATATA_TERM_COLOR_BLUE, "OpenGL");
  ...
  // GPU Vendor
  {
    ...
    fast_io::io::println (
        ...
        "  [",
        std::string_view{ abi::__cxa_demangle (typeid (Vendor).name (),
        ...
        " Device Vendor : ", PATATA_TERM_RESET, std::string_view{ Vendor });
  }
...
#if defined(__GNUC__) || defined(__MINGW64__) && !defined(__clang__)
        "  [",
        std::string_view{ abi::__cxa_demangle (typeid (Name).name (), nullptr,
                                               nullptr, nullptr) },
        "]",
#else
...
#endif
        PATATA_TERM_RESET, PATATA_TERM_BOLD,
        " Device Name : ", PATATA_TERM_RESET, std::string_view{ Name });
  }
  ...
  // OpenGL Version
  {
    ...
    std::string glversion = std::to_string (glmayorversion) + '.'
                            + std::to_string (glminorversion);

    fast_io::io::println (
        PATATA_TERM_DIM, PATATA_TERM_COLOR_GRAY0,
#if defined(__GNUC__) || defined(__MINGW64__) && !defined(__clang__)
        "  [",
        std::string_view{ abi::__cxa_demangle (typeid (glmayorversion).name (),
                                               nullptr, nullptr, nullptr) },
        "]",
#else
        "  [",
...
#endif
        PATATA_TERM_RESET, PATATA_TERM_BOLD,
        " OpenGL Version : ", PATATA_TERM_RESET, glversion);
  }

  // GLslang or GLSL Version
  {
    auto GLslang = reinterpret_cast<const char *> (
...
#if defined(__GNUC__) || defined(__MINGW64__) && !defined(__clang__)
        "  [",
        std::string_view{ abi::__cxa_demangle (typeid (GLslang).name (),
                                               nullptr, nullptr, nullptr) },
        "]",
#else
...
#endif
        PATATA_TERM_RESET, PATATA_TERM_BOLD,
        " GLslang Version : ", PATATA_TERM_RESET, std::string_view{ GLslang });
  }

  // GLAD Loader Version
  fast_io::io::println (PATATA_TERM_BOLD,
                        "  GLAD Loader Version : ", PATATA_TERM_RESET,
...

  {
    ...
    fast_io::io::print (
...
#if defined(__GNUC__) || defined(__MINGW64__) && !defined(__clang__)
        "  [",
        std::string_view{ abi::__cxa_demangle (typeid (contextflags).name (),
                                               nullptr, nullptr, nullptr) },
        "]",
#else
        ...
        "]",
#endif
        PATATA_TERM_RESET, PATATA_TERM_BOLD, " Profile : ", PATATA_TERM_RESET);
...
#if defined(__GNUC__) || defined(__MINGW64__) && !defined(__clang__)
        "  [",
        std::string_view{ abi::__cxa_demangle (typeid (accel).name (), nullptr,
                                               nullptr, nullptr) },
        "]",
#else
...
#endif
        PATATA_TERM_RESET, PATATA_TERM_BOLD,
        " Accelerated Visual : ", PATATA_TERM_RESET);
...
#if defined(__GNUC__) || defined(__MINGW64__) && !defined(__clang__)
        "  [",
        std::string_view{ abi::__cxa_demangle (typeid (buff).name (), nullptr,
                                               nullptr, nullptr) },
        "]",
#else
        "  [", std::string_view{ typeid (glGetString (buff)).name () }, "]",
#endif
        PATATA_TERM_RESET, PATATA_TERM_BOLD, " Buffer : ", PATATA_TERM_RESET);
...
#endif
        PATATA_TERM_RESET, PATATA_TERM_BOLD, " Vsync : ", PATATA_TERM_RESET);

    if (CONFIG["patata-engine"]["raccoon-renderer"]["vsync"].as<bool> ())
      {
        fast_io::io::print (PATATA_TERM_COLOR_GREEN, "Yes", PATATA_TERM_RESET);
        if (CONFIG["patata-engine"]["raccoon-renderer"]
                  ["opengl-adaptative-vsync"]
                      .as<bool> ())
...
// ImGui Version
  ...
  /*
  fast_io::io::println("\n", Bold, FSteelBlue1, "    Used Extensions", Reset,
  Dim);

  if (strcmp(reinterpret_cast<const char *>(glGetString(GL_VENDOR)),
  reinterpret_cast<const char *>("ATI Technologies Inc")) != 0) { if
  (GL_AMD_performance_monitor) fast_io::io::println("
  GL_AMD_perfomance_monitor");
  }
  fast_io::io::print(Reset);*/

#if DEBUG
/*fast_io::io::println("\n", Bold, FSteelBlue1, "    Available Extensions :",
   Reset, Dim);
        {
                ...
                for (GLint i = 0; i < NumberOfExtensions; i++)
                        fast_io::io::println("        ", std::string_view{
