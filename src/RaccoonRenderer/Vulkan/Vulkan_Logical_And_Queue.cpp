...
Patata::Graphics::RaccoonRenderer::VulkanBackend::
    CreateLogicalDeviceAndCreateQueue (void)
{
  ...
  for (uint32_t indexFlag = 0; indexFlag < QueueFamilyProperties.size ();
       indexFlag++)
    if ((QueueFamilyProperties[indexFlag].queueFlags
  ...
  const char * DeviceExtensions[]
      = { "VK_KHR_swapchain", "VK_KHR_driver_properties" };
