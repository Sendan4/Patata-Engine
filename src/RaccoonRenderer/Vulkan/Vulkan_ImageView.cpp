...
void
Patata::Graphics::RaccoonRenderer::VulkanBackend::CreateImageView (
    uint32_t & GraphicsQueueFamilyIndex)
{
  vk::ImageCreateInfo CreateImageInfo{};
  CreateImageInfo.imageType = vk::ImageType::e2D;
  CreateImageInfo.setExtent (
  ...
  CreateImageInfo.usage       = vk::ImageUsageFlagBits::eDepthStencilAttachment
                          | vk::ImageUsageFlagBits::eTransferSrc;
  // CreateImageInfo.usage = vk::ImageUsageFlagBits::eColorAttachment |
  // vk::ImageUsageFlagBits::eTransferSrc;
  ...
  vk::Result Result
      = Device.createImage (&CreateImageInfo, nullptr, &DepthImage);

  if (Result != vk::Result::eSuccess)
#if defined(_WIN64)
    fast_io::io::println (fast_io::out (), "DepthImage For BindImageMemory : ",
                          vk::to_string (Result));
#else
    fast_io::io::println (
        PATATA_TERM_BOLD,
        "DepthImage For BindImageMemory : ", PATATA_TERM_RESET,
  ...
  for (uint32_t i = 0; i < SwapChainBuffers.size (); i++)
    {
      ...
      Result
          = Device.createImageView (&CreateImageViewInfo, nullptr, &ImageView);

      if (Result != vk::Result::eSuccess)
#if defined(_WIN64)
        fast_io::io::println (fast_io::out (), "SwapChainBuffer ImageView : ",
                              vk::to_string (Result));
#else
        fast_io::io::println ("SwapChainBuffer ImageView : ",
                              vk::to_string (Result));
#endif
      else
#if defined(_WIN64)
